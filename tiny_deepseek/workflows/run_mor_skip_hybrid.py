...
def main() -> None:
    ...
    parser.add_argument(
        "--mor-checkpoint",
        default=(
            "artifacts/experiments/mor_comparison_seed42/mor_middle_cycle_R3_seed42/"
            "checkpoints/best_val_loss.pt"
        ),
    )
    parser.add_argument(
        "--experiments-dir", default="artifacts/experiments/mor_comparison_seed42"
    )
    ...
    if not complete:
        ...
        run(
            [
                sys.executable, "-m", "tiny_deepseek.training.mor_skip", *source, *resume,
    ...
    if not complete:
        ...
        run(
            [
                sys.executable, "-m", "tiny_deepseek.training.mor_skip_grpo", *source, *resume,
