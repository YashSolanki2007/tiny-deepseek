"""Train the MLA+RoPE SkipLayer+MTP math model with dense or MoE FFNs."""
...
from tiny_deepseek.data.math import MathData
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--experiment-dir", default="artifacts/experiments/math_v2_seed42/supervised")
    ...
    parser.add_argument(
        "--dense-ffn",
        action="store_true",
        help="Replace MoE layers with one ordinary d-ff-wide dense FFN per block.",
    )
    ...
    parser.add_argument(
        "--defer-readiness",
        action="store_true",
        help="Leave answer/pass@k evaluation to the readiness CLI module.",
    )
...
def main() -> None:
    ...
    config = ModelConfig(
        ...
        d_ff=args.d_ff,
        dropout=0.05,
        model_type=("sparse_mtp_mla" if args.dense_ffn else "sparse_moe_mtp_mla"),
        router_type="linear",
        router_dim=32,
    ...
    try:
        for step in range(start_step, args.max_steps):
            ...
            while processed_examples < effective_batch_size:
                ...
                moe_router_entropy = (
                    output.moe_router_entropy
                    if output.moe_router_entropy is not None
                    else output.lm_loss.new_zeros(())
                )
                total = (
                    output.lm_loss
                    + config.mtp_loss_coefficient * output.mtp_loss
                    + config.moe_aux_loss_coefficient * moe_aux_loss
                ...
                current = {
                    ...
                    "moe_router_entropy": float(moe_router_entropy.item()),
                    "total_loss": float(total.item()),
                    **{
                        key: float(route.get(key, 0.0)) for key in (
