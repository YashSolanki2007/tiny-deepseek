...
```bash
python run_paper_experiments.py --preset smoke
```

Meaningful Tiny Shakespeare comparison:

```bash
python run_paper_experiments.py --preset core --seeds 42 43 44
```
...
```bash
python run_paper_experiments.py --preset core --paper-learning-rate 0.01
```

Small analogue of the depth/density rows in Table 1:

```bash
python run_paper_experiments.py --preset table1 --seeds 42 43 44
```

TensorBoard:

```bash
tensorboard --logdir experiments/paper_core
```
