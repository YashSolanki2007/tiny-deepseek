...
def main() -> None:
    ...
    experiment_root = Path(
        ...
        or f"experiments/skiplayer_moe_mtp_{'mla_rope' if args.attention == 'mla' else 'mha'}_seed{args.seed}"
    )
    ...
    if not (supervised / "summary.json").exists():
        run(
            [
                sys.executable, str(root / "train_moe_mtp.py"),
    ...
    if not (grpo / "summary.json").exists():
        run(
            [
                sys.executable, str(root / "train_paper_grpo.py"),
